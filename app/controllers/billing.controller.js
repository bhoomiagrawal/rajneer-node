...
const calculateWaterCharges = async ({
  ...
  try {
    ...
    console.log("tariffs value is :-", tariffs, billPayload);
    if (!tariffs || tariffs.length === 0) {
      console.log('error:::::', "No water tariff configurations found for the given category/connection size or bulk status.")
      // throw new Error(
    ...
    for (const tariff of tariffs) {
      ...
      let slabMax =
        slab?.max_consumption == null || slab?.max_consumption == ""
          ? Infinity
          : slab?.max_consumption;
      ...
      if (!isBulk) {
        if (consumption <= slabMax) {
          console.log(
            "entring in consumption <= slabMax part:::::: consumption is,",
            consumption,
            "and slabMax is::::",
            slabMax,
            "::::::"
          ...
          console.log(
            "Slabmax values is:::",
            slabMax,
            "   and previousmax vlues is:::: ",
            previousMax,
            "   so the difference is::: ",
            consumption - previousMax,
            "   : for rate per thousand is::",
            ratePerThousand,
            "  ::"
          );
...
exports.generateBill = async (req, res) => {
  ...
  try {
    if (!billingData || billingData.length === 0) {
      // Check if billingData array is empty.
      return res
        .status(400)
        .json({ message: "Invalid payload: No data provided" });
    }

    const generatedBills = [];
    // Loop through each billing object in the array and process them.
    for (const billPayload of billingData) {
      // Changed: Loop through the billingData array
      ...
      for (const month of monthData) {
        ...
        const data = await calculateWaterCharges({
          category_id,
          connection_size_id,
          consumption,
          connection_type_id,
        ...
        const meterServiceCharge =
          meterServiceChargeTariff?.ratePerThousand || 0;
