...
exports.createReadingSheet = async (req, res) => {
  try {
    ...
    // Extract unique chk & grup combinations with consumer count
    const groupedConsumers = consumers.reduce((acc, consumer) => {
      const key = `${consumer.chk}-${consumer.grup}`;
      if (!acc[key]) {
        acc[key] = { chk: consumer.chk, grup: consumer.grup, cin_numbers: [], consumerCount: 0,chkGrup:[]  };
      }
      acc[key].cin_numbers.push(consumer.cin_number);
      acc[key].consumerCount++;
      return acc;
    }, {});
    ...
    // Loop through each chk & grup combination
    for (const key in groupedConsumers) {
      const { chk, grup, cin_numbers, consumerCount } = groupedConsumers[key];
      ...
      readingSheets.push({
        ...
        grup,
        consumerCount, // Added consumer count
        reading_from_date,
        reading_to_date,
        bill_issue_date,
        due_date_cash,
    ...
    // Step 3: If reading sheets already exist, include consumer count and return response
    if (existingReadingSheets.length > 0) {
      const updatedSheets = existingReadingSheets.map((sheet) => {
        const key = `${sheet.chk}-${sheet.grup}`;
        return {
          ...sheet.dataValues,
          consumerCount: groupedConsumers[key] ? groupedConsumers[key].consumerCount : 0,
          chkGrup: key
        };
      });

      return res.status(200).json({
        message: "Reading sheets already exist",
        data: updatedSheets,
      });
    }

    // Insert new reading sheets
    if (readingSheets.length > 0) {
      await ReadingSheet.bulkCreate(readingSheets);
      return res.status(201).json({
        message: "Reading sheets created successfully",
        data: readingSheets,
      });
    } else {
      return res.status(400).json({ message: "No new reading sheets were created." });
